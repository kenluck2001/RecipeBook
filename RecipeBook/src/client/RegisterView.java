...
public class RegisterView extends VerticalLayout {
    ...
    public RegisterView(RecipebookApplication ra) {
        application = ra;
...
/*        Button discardChanges = new Button("Discard changes",
        ...
        Button register = new Button("Register", new Button.ClickListener() {
            public void buttonClick(ClickEvent event) {
                try {
                    personForm.commit();
                    if (Query.checkUser(person.getUsername())) {
                        getWindow().showNotification("Username already taken");
                        person.setUsername("");
                    } else if (person.getPassword().equals(person.getPasswordAgain()))
                    {
                        ...
                        query.InsertUser(person.getUsername(),
                                person.getPassword());
                        User user = Query.login(person.getUsername(),
                                person.getPassword());
                        if (user != null) {
                            application.login();
                            Status.login(user);
                            getWindow().showNotification(
                                    "User " + person.getUsername() + " saved",
                                    Notification.TYPE_HUMANIZED_MESSAGE);
                        } else {
                            getWindow().showNotification("Registration failed!",
                                    Notification.TYPE_HUMANIZED_MESSAGE);
                            application.logout();
                        }
